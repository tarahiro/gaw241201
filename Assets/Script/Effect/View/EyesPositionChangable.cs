...
{
    public class EyesPositionChangable : MonoBehaviour, IPositionChangable
    {
       ...
       public async UniTask ChangePosition(EyeMoveView.EyePositionKey key)
        {
            await LMotion.Create(transform.position, TargetPosition(key), c_moveTime).BindToPosition(transform);
        }
